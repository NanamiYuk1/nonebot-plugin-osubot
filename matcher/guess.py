...
from ..draw.osu_preview import render_preview
...
async def _(
    ...
    try:
        ...
        game_manager.pop_timer(GameType.PIC, session_id)
        game_manager.pop_game(GameType.PIC, session_id)
        game_manager.reset_hint(GameType.PIC, session_id)
        await guess_pic.finish(f"背景图获取失败，本次猜歌名为: {selected_score.beatmapset.title_unicode}")
...
async def _(
    ...
    # 【改】谱面猜歌渲染统一走 render_preview（core 二进制，失败自动 fallback）：
    #   mode 0 -> gif；mode 1/3 -> png；mode 2(catch) -> png 且带该成绩的真实 mods
    if mode == "2":
