...
async def draw_score(
    ...
    *,
    ...
    if project == "bp":
        ...
        # recent/pr 需要按 mods/筛选条件过滤，放宽拉取窗口再过滤，避免窗口内凑不满
        fetch_limit = max(best, 100) if (mods or search_condition) else best
        if project == "pr":
            scores = await get_user_scores(
                uid,
                mode,
            ...
            scores = await get_user_scores(uid, mode, "recent", source=source, legacy_only=not is_lazer, limit=fetch_limit)
    if not scores:
