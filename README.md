...
## 多 bot 部署：反向 WebSocket 与消息查重

本插件单账号部署无需额外配置。但若你使用**多个 QQ 账号（主号 + 分身）**同时接入同一个 NoneBot 进程，同一条群消息会被推送给所有在该群的分身账号，导致每个 bot 都执行指令、都回复一份。本节说明连接方式与配套的消息查重插件。
...
### 2. 消息查重插件（nonebot_plugin_primary_bot）

消息查重由部署配套插件 `nonebot_plugin_primary_bot` 提供（独立于本插件，需自行放入 NoneBot 项目的 `plugins/` 目录，`nonebot.load_plugins("plugins")` 会自动加载）。

它在消息进入匹配器之前做**会话级去重**——同一物理群消息只允许一个 bot 处理并回复：

- **主号在场**：优先由主号回复，分身等待约 1 秒确认主号已接管后让路；
- **主号不在该群**（或尚未被观察到）：多个分身先到先得，只回复一次；
- **私聊**天然不会重复，不参与去重。

> ⚠️ 去重键**不能使用 `message_id`**：NapCat 等实现为每个账号的连接独立编号，同一条物理消息在
> 主号与分身上看到的 `message_id` 不同（例如同一消息一个账号看到 `979523017`、另一个看到
> `1523281098`），按 message_id 去重会完全不生效。插件改用 **群号 + 发送者 QQ + 发送时间 + 消息内容**
> （剔除 `[CQ:reply,id=...]` 回复段，其 id 同样按账号而异）作为跨账号稳定的指纹。
...
```ini
# 主号 QQ：同会话去重时优先由主号回复；不填则回退使用 SUPERUSERS 中的第一个 QQ
MAIN_BOT_QQ=3357098112
# 超级用户（管理员）QQ
SUPERUSERS=["751733273"]
```

### 4. 注意事项

- 修改代码 / 配置后必须**重启** NoneBot 进程才生效（运行中的进程内存里仍是旧模块）；
- `/mp`（多人比赛）多图发送已做多 bot 适配：主号走合并转发，分身降级为逐张发送，避免发错账号；
- 主号在场时，分身会对该群**所有**群消息让路（包括非指令消息），这是设计行为；
- 主号识别优先级：环境变量 `MAIN_BOT_QQ` > `SUPERUSERS` 中的第一个 QQ。
